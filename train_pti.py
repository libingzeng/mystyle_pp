...
def generate_from_anchors_pti(generator, sample, output_path):
    generator.eval()
    img = generator(sample.w_code.cuda(), noise_mode='const', force_fp32=True)
    io_utils.save_images(img, output_path.joinpath(sample.name))
...
def parse_args(raw_args=None):
    ...
    parser.add_argument('--tune_steps', type=int, default=1000)
...
def process_args(args):
    ...
    args.pti_images_dir = args.output_dir.joinpath('pti_images')
    args.pti_images_dir.mkdir(exist_ok=True, parents=True)
...
def main(raw_args=None):
    ...
    for sample in dataset:
        print('PTI fine-tune generator on image: {}'.format(sample.name))
        
        args.pti_sample_dir = args.pti_images_dir.joinpath('{}'.format(sample.name))
        args.pti_sample_dir.mkdir(exist_ok=True, parents=True)
        args.debug_output_dir = args.pti_sample_dir.joinpath('debug')
        args.debug_output_dir.mkdir(exist_ok=True, parents=True)
      
        generator = io_utils.load_net(args.generator_path)

        if args.verbose:
            generate_from_anchors_pti(generator, sample, args.debug_output_dir.joinpath('before'))

        network_tuner = Tuner(args.device, generator, args.debug_output_dir.joinpath('during') if args.verbose else None, tune_steps=args.tune_steps)
        network_tuner.reconstruct_pti(sample)

        torch.save(network_tuner.generator, args.pti_sample_dir.joinpath(f'mystyle_model.pt'))

        if args.verbose:
            generate_from_anchors_pti(generator, sample, args.debug_output_dir.joinpath('after'))
