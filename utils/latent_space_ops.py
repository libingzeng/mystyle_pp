...
def sample_from_P0(anchors: torch.FloatTensor, num_points_to_sample: int, num_anchors_for_sample: int = 3):
    ...
    for i in range(num_points_to_sample):
        # lz, randomly choose 'num_anchors_for_sample' axes (integers) from 'anchors.shape[0]' axes (integers).
        # by codes, line 1, line2, and line3.
        indices[i, :] = np.random.choice(anchors.shape[0], size=num_anchors_for_sample, replace=False) # line1
        scalars[i, indices[i]] = tmp_scalars[i] # line2

    # pdb.set_trace()
    
    if anchors.dim() == 4:
        points = torch.einsum('pN, Nolk ->  polk', scalars, anchors) # line3
    elif anchors.dim() == 2:
...
def sample_from_P0_view_pair(anchors_dict, num_points_to_sample: int, num_anchors_for_sample: int = 3):
    anchors = anchors_dict[list(anchors_dict.keys())[0]]
    num_anchors = anchors.shape[0]
    ...
    points_view_pair_dict = {}
    for view in anchors_dict.keys():
        anchors_view = anchors_dict[view]
        if anchors_view.dim() == 4:
            points = torch.einsum('pN, Nolk ->  polk', scalars, anchors_view) # line3
        elif anchors_view.dim() == 2:
            points = scalars @ anchors_view
        else:
            raise ValueError('Anchors are in unknown format')
        points_view_pair_dict[view] = points.to('cuda')
    
    return points_view_pair_dict
