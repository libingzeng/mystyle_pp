...
class ImageReps:
    ...
    alpha_opt: torch.FloatTensor = None
    deltas_opt: torch.FloatTensor = None
    ...
    def set(self, latent=None, alpha_opt=None, deltas_opt=None, alpha=None, recon_img=None):
        if latent is not None:
            self.w_code = latent.detach().cpu()
        if alpha is not None:
            self.alpha = alpha.detach().cpu()
        if alpha_opt is not None:
            self.alpha_opt = alpha_opt.detach().cpu()
        if deltas_opt is not None:
            self.deltas_opt = deltas_opt.detach().cpu()
    ...
    def save_latent(self, output_path: Path):
        ...
        io_utils.save_latents(self.alpha_opt, output_path.joinpath('alpha_opt', self.name).with_suffix('.pt'))
        io_utils.save_latents(self.deltas_opt, output_path.joinpath('deltas_opt', self.name).with_suffix('.pt'))
    ...
    def save_recon(self, output_path: Path):
        io_utils.save_images(self.recon_img, output_path.joinpath('out_image', self.name + '_recon').with_suffix('.jpg'))
