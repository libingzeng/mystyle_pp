...
class Tuner(BaseReconstructor):
    def __init__(self, device, generator, debug_out_path=None, l2_weight=hyperparams.l2_weight, tune_steps=hyperparams.tune_steps):
        super().__init__(device, generator, tune_steps, debug_out_path, l2_weight)
    ...
    def reconstruct(self, dataset):
        for sample in dataset: 
            ...
            break
        writer = SummaryWriter(log_dir='runs/{}_{}_mystyle'.format(sample_name.split('_')[0], sample_name.split('_')[1]))
    ...
    def reconstruct_pti(self, sample):
        for step in tqdm(range(self.num_steps)):
            tot_loss = 0
            to_visualize = self.need_visualize(step)
